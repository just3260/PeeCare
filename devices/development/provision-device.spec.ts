...
describe('provision-device', () => {
  ...
  it('deletes a newly created principal when ACL mutation fails', async () => {
    ...
    expect(emqx.deleteCredential).toHaveBeenCalledWith('device-68E274BD2A58')
  ...
  it('deletes the principal when the one-time handoff write fails', async () => {
    ...
    expect(deps.emqx.deleteCredential).toHaveBeenCalledWith('device-68E274BD2A58')
    expect(deps.emqx.deleteAcl).toHaveBeenCalledWith('device-68E274BD2A58')
  ...
  it('uses only the HTTPS /api/v5 built-in database access-control endpoints', async () => {
    ...
    await adapter.putAcl('device-68E274BD2A58', [{ permission: 'deny', action: 'all', topic: '#' }])
    await adapter.deleteAcl('device-68E274BD2A58')

    expect(fetchImpl.mock.calls.map(([url]) => url)).toEqual([
      'https://emqx.development.example/api/v5/authentication/password_based%3Abuilt_in_database/users',
      'https://emqx.development.example/api/v5/authorization/sources/built_in_database/rules/users/device-68E274BD2A58',
      'https://emqx.development.example/api/v5/authorization/sources/built_in_database/rules/users/device-68E274BD2A58',
    ])
    expect(JSON.parse(fetchImpl.mock.calls[0][1].body)).toEqual({
      user_id: 'device-68E274BD2A58',
      password: 'generated-device-password',
      is_superuser: false,
    })
  })
  ...
  it('emits fixed-shape sanitized JSON even if a caller supplies extra secret-bearing fields', () => {
    ...
    expect(output.write).toHaveBeenCalledWith(
      '{"mode":"apply","deviceId":"68E274BD2A58","principal":"device-68E274BD2A58","status":"applied","verifications":["inventory"]}\n',
    )
