...
const firmware = {
  ...
  payloadIdentity: { deviceId: 'PC-000001', productModel: 'pc-mini' },
}
...
describe('registry alignment', () => {
  it('accepts the approved enabled petcare-c7483/devices/PC-000001 document', () => {
  ...
  it('uses Application Default Credentials for one exact read-only document lookup', async () => {
    ...
    await expect(
      reader.readDevice({ projectId: 'petcare-c7483', documentPath: 'devices/PC-000001' }),
    ).resolves.toEqual(enabledRegistryDocument)
    expect(Firestore).toHaveBeenCalledWith({ projectId: 'petcare-c7483' })
    expect(doc).toHaveBeenCalledWith('devices/PC-000001')
    expect(get).toHaveBeenCalledOnce()
  })

  it('performs zero EMQX writes for the specified disabled PC-000001 example', async () => {
